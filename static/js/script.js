$(function () {
    ...
    function initTopNav() {
        ...
        if ($(this).scrollTop() >= 50) {
            $('#search').css({
                right: getInitSearchPosition() + 200 + 'px',
                top: '5px',
            })
        } else {
            $('#search').css({
                right: getInitSearchPosition() + 'px',
                top: '85px',
            })
        }
        if ($(this).scrollTop() >= 200) {
            $('.bg-wrap').css({ backgroundColor: 'rgba(0,0,0,.6)' })

        } else {
            $('.bg-wrap').css({ backgroundColor: 'rgba(0,0,0,.2)' })

        }
    }
    //设置搜索框位置
    function getInitSearchPosition() {
        var screenWidth = $(window).width();
        var xPosition = (screenWidth - 980) / 2
        return xPosition
    }
    sideBar();
    ...
    $sideBar_btn.click(function () {
        ...
        $("html,body").stop().animate({ scrollTop: $mainCont_item.eq(index).offset().top - 42 });
